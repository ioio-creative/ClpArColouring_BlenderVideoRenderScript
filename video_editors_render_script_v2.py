...
# Added by Christopher Wong, 2017-07-11
# block and enable print to stdout
...
# end of block and enable print to stdout
...
# Added by Christopher Wong, 2017-07-10
...
# Added by Christopher Wong, 2017-09-15
def is_even(an_int):
    return an_int % 2 == 0

def is_positive(an_int):
    return an_int > 0

# Added by Christopher Wong, 2017-07-14	
def is_absolute_path(path_str):
    return path_str.find("C:") == -1 or path_str.find("c:") == -1

# Added by Christopher Wong, 2017-07-14	
def cleanse_path(old_path_str):
    new_path_str = old_path_str
    # Precaution step
    if my_platform == "Windows": # Windows 10
        new_path_str = old_path_str.replace("\\", "/")
        if not is_absolute_path(new_path_str):  # relative path
            new_path_str = "//" + new_path_str
    return new_path_str

# Added by Christopher Wong, 2017-07-10
def read_in_command_line_arguments():
    ...
    parser.add_argument("-t", "--texture", dest = "texture_path", metavar = 'FILE',
    ...
    # Added by Christopher Wong, 2017-09-05
    parser.add_argument("-asc", "--allowsinglecore", dest = "is_allow_single_core", type = int,
            help = "Is allow single core? 1 = Yes, 0 = No")

    # Added by Christopher Wong, 2017-09-12
    parser.add_argument("-rc", "--reservedcores", dest = "num_of_reserved_cores", type = int,
            help = "No. of reserved cores")

    # Added by Christopher Wong, 2017-09-15
    parser.add_argument("-xr", "--xresolution", dest = "x_resolution", type = int,
            help = "X-resolution of output")
    parser.add_argument("-yr", "--yresolution", dest = "y_resolution", type = int,
            help = "Y-resolution of output")

    # Added by Christopher Wong, 2017-09-29
    parser.add_argument("-fr", "--framerate", dest = "frame_rate", type = int,
    ...
    if args.start_frame and not (is_int(args.start_frame) and is_positive(args.start_frame)):
        print("Error: --startframe=\"some integer\" argument not positive integer, aborting.")
        parser.print_help()
        return

    if args.end_frame and not (is_int(args.end_frame) and is_positive(args.end_frame)):
        print("Error: --endframe=\"some integer\" argument not positive integer, aborting.")
        parser.print_help()
        return

    # Added by Christopher Wong, 2017-09-05
    if args.is_allow_single_core and not is_int(args.is_allow_single_core):
        print("Error: --allowsinglecore=\"some integer\" argument not integer, aborting.")
        parser.print_help()
        return

    # Added by Christopher Wong, 2017-09-12
    if args.num_of_reserved_cores and not is_int(args.num_of_reserved_cores):
        print("Error: --reservedcores=\"some integer\" argument not integer, aborting.")
        parser.print_help()
        return

    # Added by Christopher Wong, 2017-09-15
    if args.x_resolution and not (is_int(args.x_resolution) and is_positive(int(args.x_resolution)) and is_even(int(args.x_resolution))):
        print("Error: --xresolution=\"some integer\" argument not positive even integer, aborting.")
        parser.print_help()
        return
    
    if args.y_resolution and not (is_int(args.y_resolution) and is_positive(int(args.y_resolution)) and is_even(int(args.y_resolution))):
        print("Error: --yresolution=\"some integer\" argument not positive even integer, aborting.")
        parser.print_help()
        return

    # Added by Christopher Wong, 2017-09-29
    if args.frame_rate and not is_positive(int(args.frame_rate)):
        print("Error: --framerate=\"some integer\" argument not positive integer, aborting.")
        parser.print_help()
        return

    # Modified by Christopher Wong, 2017-09-29
    return cleanse_path(args.texture_path), cleanse_path(args.render_path), int(args.start_frame), int(args.end_frame), int(args.is_allow_single_core) == 1, int(args.num_of_reserved_cores), int(args.x_resolution), int(args.y_resolution), int(args.frame_rate)

# Modified by Christopher Wong, 2017-09-29
texture_path, render_path, start_frame, end_frame, is_allow_single_core, num_of_reserved_cores, x_resolution, y_resolution, frame_rate = read_in_command_line_arguments()
...
# Modified by Chris, 2017-09-12
reserved_cpu_logical_cores = num_of_reserved_cores
...
# Added by Christopher Wong, 2017-09-26
blendfile_override_setting += "    scene.render.resolution_x = " + str(x_resolution) + "\n"
blendfile_override_setting += "    scene.render.resolution_y = " + str(y_resolution) + "\n"

# Added by Christopher Wong, 2017-09-29
blendfile_override_setting += "    scene.render.fps = " + str(frame_rate) + "\n"

# Added by Christopher Wong, 2017-07-11
# Setting frame_start and frame_end here somehow does not work
# blendfile_override_setting += "    scene.frame_start = " + str(start_frame) + "\n"
# blendfile_override_setting += "    scene.frame_end = " + str(end_frame) + "\n"

# Added by Christopher Wong, 2017-07-14
# The following statement is specific to the Blender file
#blendfile_override_setting += "bpy.data.images['world'].filepath = '" + texture_path + "'\n"

# Added by Christopher Wong, 2017-08-22
# The following statement is specific to the Blender file
blendfile_override_setting += "bpy.data.images['Map #1.009'].filepath = '" + texture_path + "'\n"
blendfile_override_setting += "bpy.data.images['Map #1.001'].filepath = '" + texture_path + "'\n"

# Setting frame_start and frame_end
myScene = bpy.data.scenes["Scene"]
myScene.frame_start = start_frame
myScene.frame_end = end_frame
# Added by Christopher Wong, 2017-09-15, (Note: somehow the following two lines are not working.)
#myScene.render.resolution_x = x_resolution
#myScene.render.resolution_y = y_resolution
...
# Added by Christopher Wong, 2017-07-11
audio_and_video = render_path  # render_path is read from command line arguments
...
if not force_one_instance_render:                                              #  | When force_one_instance_render = True, we render with only 1 blender instance.

    # Added by Christopher Wong, 2017-09-05
    if is_allow_single_core and logical_cores_available == 1:
        cores_enabled = 1
        permit_scene_strips = True  # same as else case for (if not force_one_instance_render)

    # Changed if to elif by Christopher Wong, 2017-09-05
    elif logical_cores_available == 1:
...
# Commented out by Christopher Wong, 2017-11-07
'''
...
if not blender_image_sequence:
    ...
    # Modified by Christopher Wong, 2017-07-24
    if is_absolute_path(audio_and_video):
        joined_video_with_audio = audio_and_video
    else:
...
print(blendfile_override_setting)